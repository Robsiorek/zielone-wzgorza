...
═══════════════════════════════════════════════════════════════════
CURRENT PRODUCTION STATE (stan faktyczny na 14.04.2026)
═══════════════════════════════════════════════════════════════════
...
- ✅ Testy regresyjne: 57/57 PASS (scripts/test-critical.sh + test-b1/b2/b3/b4)
...
- ✅ B4 Property Content (PropertyContent singleton 23 pól, TrustBadge CRUD+reorder, FaqItem CRUD+reorder, public API frozen shape, admin /admin/property-content z 6 SectionCards + HelpBox, DS v1.8 §31)
- ✅ B5a Infrastruktura (booking.zielonewzgorza.eu, SSL, Nginx proxy)
...
udogodnienia domków (B1/B2/B3) i treści obiektu (B4) wdrożone.
Infrastruktura silnika rezerwacyjnego (B5a) gotowa: booking.zielonewzgorza.eu.
Brakuje: silnik rezerwacyjny UI (B5a frontend), embed/modal (B5b),
...
**Property Content (Warstwa B — ✅ B4 wdrożone):**

- GET /api/property-content — Odczyt PropertyContent singleton (frozen shape, 23 pól)
- PATCH /api/property-content — Upsert (create-or-update, partial patch)
- GET /api/property-content/trust-badges — Lista trust badges (sorted by position)
- POST /api/property-content/trust-badges — Create badge (iconKey validation, max(position)+1)
- PATCH /api/property-content/trust-badges/[id] — Update badge (partial)
- DELETE /api/property-content/trust-badges/[id] — Delete badge (hard)
- PATCH /api/property-content/trust-badges/reorder — Reorder (7-step, ADR-18)
- GET /api/property-content/faq — Lista FAQ items (sorted by position)
- POST /api/property-content/faq — Create FAQ item (max(position)+1)
- PATCH /api/property-content/faq/[id] — Update FAQ item (partial)
- DELETE /api/property-content/faq/[id] — Delete FAQ item (hard)
- PATCH /api/property-content/faq/reorder — Reorder (7-step, ADR-18)

**Publiczne API (E1 + E2 + B4, bez auth, z rate limiterem):**
...
- GET /api/public/resources-catalog — Katalog zasobów dla widgetu (E2)
  (B1/B2/B3 ✅: images, shortDescription, beds, areaSqm, bedroomCount, bathroomCount, amenities)
- GET /api/public/resources/[id] — Szczegóły zasobu (B2 backend ✅, B5 UI).
  Zwraca: longDescription, images, beds, variants, amenities.
- GET /api/public/property-content — Treści obiektu (B4 ✅). Frozen shape:
  propertyContent (23 pól) + trustBadges[] (active, 5 pól) + faqItems[] (active, 4 pola).
  Filtr isActive, sorted by position. Bez propertyId/timestamps w response.
...
🟢 Warstwa B (zamknięta): B1 Media ✅, B2 Content ✅, B3 Amenities ✅, B4 Property Content ✅.
  B5a infra ✅ (booking.zielonewzgorza.eu). B5a frontend w realizacji.
...
B4 ✅ Property content: PropertyContent (singleton, 23 pól), TrustBadge (CRUD+reorder),
  FaqItem (CRUD+reorder). Admin /admin/property-content "Informacje dla gości" z 6
  SectionCards + per-section HelpBox (DS §31). Public API frozen shape.
  16 testów T42-T57. Design System v1.8. Commity: b47cb3f, 2853d39.
...
v2.5 (14.04.2026) — B4 Property Content: 3 nowe modele (PropertyContent
  singleton 23 pól, TrustBadge, FaqItem), 12 endpointów admin (singleton
  CRUD, trust badges CRUD+reorder, FAQ CRUD+reorder), 1 endpoint public
  (frozen shape: content + active badges + active FAQ, bez isActive/
  propertyId/timestamps). Admin strona /admin/property-content "Informacje
  dla gości" z 6 SectionCards, per-section save (repopulateSection —
  nie resetuje dirty pól innych sekcji), DnD reorder, SlidePanel CRUD,
  icon picker (reuse B3), ConfirmDialog delete. Per-section HelpBox
  (DS §31) — kontekstowy panel pomocy po prawej, fade animacja,
  hidden xl:block. Prisma migration (migrate deploy, nie db push).
  Rate limiter: propertyContentLimiter 60 req/min (reuse createRateLimiter).
  Markdown-safe rendering gotowe pod B5. Design System v1.8 (§31 HelpBox,
  live preview w /admin/design-system). SectionCard: onToggle prop
  (backwards compatible). Bilans: 57/57 PASS (18+6+7+10+16).
  B5a infra: booking.zielonewzgorza.eu (DNS home.pl, SSL certbot, Nginx
  proxy→:3000). Architektura B5a zatwierdzona: Headless Booking Engine,
  front-agnostic, WordPress = pierwszy klient (nie jedyny).
  Backlog: legalName/displayName, NIP/KRS, typ VAT, faktura toggle,
  formaty regionalne (po B5), multi-location (daleka przyszłość).
