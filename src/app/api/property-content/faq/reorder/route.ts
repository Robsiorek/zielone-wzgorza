/**
 * PATCH /api/property-content/faq/reorder — Reorder FAQ items.
 *
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (existing.length !== ids.length) {
      ...
      return apiError(`Nieznane FAQ: ${missing.join(", ")}`, 400, "VALIDATION");
    }

    // Step 6: Same propertyId scope
    const propertyIds = new Set(existing.map((f) => f.propertyId));
    if (propertyIds.size > 1) {
      return apiError("Pytania FAQ należą do różnych obiektów", 400, "VALIDATION");
    }

    // Step 7: Completeness check — payload count === DB count
    const propertyId = existing[0].propertyId;
    const totalInDb = await prisma.faqItem.count({ where: { propertyId } });
    if (ids.length !== totalInDb) {
      return apiError(
        `Payload zawiera ${ids.length} pytań, ale obiekt ma ${totalInDb}. Reorder wymaga pełnej listy.`,
        400,
        "VALIDATION"
      );
    }
