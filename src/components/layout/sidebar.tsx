...
const menu: MenuGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Informacje dla gości", href: "/admin/property-content", icon: FileText },
      {
      ...
      <aside className={cn(
              ...
              <div className={cn(isMobile ? "space-y-2" : "space-y-0.5")}>
                {group.items.map((item) => {
                    ...
                    return (
                      <div key={item.title}>
                        <button onClick={() => toggle(item.title)}
                          className={cn(
                            "group flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-[14px] font-medium transition-all duration-200",
                            parentActive
                          ...
                          <Icon className={cn(
                            ...
                            parentActive ? "text-primary" : "text-muted-foreground group-hover:text-foreground"
                          )} />
                            ...
                            {item.children.map(child => {
                              ...
                              return (
                                <Link key={child.title} href={child.href} onClick={() => handleNav(child.href)}
                                  className={cn(
                                    "group/child flex items-center gap-2 rounded-lg px-2.5 py-2 text-[13px] transition-all duration-200",
                                    childActive
                                      ? "text-foreground font-semibold bg-primary/10"
                                      : "text-muted-foreground font-medium hover:text-foreground hover:bg-muted"
                                  )}>
                                  <ChildIcon className={cn("h-4 w-4 shrink-0 transition-colors duration-200",
                                    childActive ? "text-primary" : "text-muted-foreground group-hover/child:text-foreground"
                                  )} />
                  ...
                  return (
                    <Link key={item.title} href={item.href || "#"} onClick={() => handleNav(item.href)}
                      className={cn(
                        "group flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-[14px] font-medium transition-all duration-200",
                        itemActive
                          ? "text-foreground bg-primary/10"
                          : "text-muted-foreground hover:text-foreground hover:bg-muted"
                      )}>
                      <Icon className={cn(
                        "h-4 w-4 shrink-0 transition-colors duration-200",
                        itemActive ? "text-primary" : "text-muted-foreground group-hover:text-foreground"
                      )} />
        ...
        <div className={cn("shrink-0 border-t border-border/40", isMobile ? "px-4 py-3" : "px-3 py-3")}>
          <Link href="/admin/design-system"
            ...
            {(!collapsed || isMobile) && <span className="text-[14px] font-medium">Design System</span>}
          </Link>
        </div>

        {/* ── Engine UI Lab link ── */}
        <div className={cn("shrink-0", isMobile ? "px-4 pb-3" : "px-3 pb-3")}>
          <Link href="/admin/engine-ui-lab"
            className={cn(
              "flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-muted-foreground hover:bg-muted hover:text-foreground transition-all duration-200",
              pathname === "/admin/engine-ui-lab" && "bg-muted text-foreground",
              collapsed && !isMobile && "justify-center px-0"
            )}>
            <Sparkles className="h-4 w-4 shrink-0" />
            {(!collapsed || isMobile) && (
              <>
                <span className="text-[14px] font-medium">Engine UI Lab</span>
                <span className="ml-auto text-[9px] bg-muted px-1.5 py-0.5 rounded font-bold">LAB</span>
              </>
            )}
          </Link>
        </div>
        ...
        <div className="px-3 py-3 border-t hidden lg:block shrink-0">
          <button onClick={onToggle}
            ...
            {!collapsed && <span className="ml-2 text-[13px] font-medium">Zwiń menu</span>}
          </button>
        </div>
      </aside>
    </>
